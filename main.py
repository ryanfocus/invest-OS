...
def _place_entry_order(
    ...
    另外三道在呼叫端：**今天已經進過場**、**狀態檔讀不懂**（都在 `run_entry`
    的更前面——依 SPEC 的進場流程，重複執行保護是第 2 步，排在登入與發報之前；
    放到這裡才檢查的話，重跑會再發一則一模一樣的 Discord），以及
    **這一班太晚了**（在 `_run_strategy` 呼叫這裡的前一行，位置刻意排在
    發報與觀測之後，那兩件事不該因為晚了而消失）。
    """
